...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //normal cold flows example
        //viewModel.flowUsage()

        // hot flows example - mutuable shared flow
        // viewModel.hotFlowUsage()

        // hot flows example- mutuable state flow -which keeps buffer of recent last item
        viewModel.hotFlowUsage()

    }

    fun setBannerListing(bannerList: ArrayList<BannerDataModel>) {
        if (!bannerList.isNullOrEmpty() && bannerList.size > 0) {


            //setting banner adapter data
            bannerAdapter = BannerAdapter(
                this, bannerList, binding.vpBanner, swipeDelay * 1000.toLong()
            )
    ...
    fun setObservers() {
        ...
        lifecycleScope.launch {
            viewModel.showBottomSheetDialogLiveData.collect {
    ...
    fun setMessageData(messages: MESSAGES) {
        if (messages.equals(MESSAGES.BOTTOM_SHEET_DATA_FOUND)) {
        ...
        } else if (messages.equals(MESSAGES.BOTTOM_SHEET_NO_DATA_FOUND)) {
